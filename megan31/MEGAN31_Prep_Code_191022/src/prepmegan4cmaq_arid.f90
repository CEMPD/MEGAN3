...
!----------------------------------------------------
!----------------------------------------------------
   real, allocatable :: arid(:,:)                         ! arid daily index value
...
!-----read arid daily index----------------------------------------------
      scale_factor  = 1.
      inpname       = 'soil_climate_arid.nc'
      varname       = 'arid'
      missing_value = -999
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_arid'
!write to arid.csv file---------------------
      CALL write_arid
...
!---------------------------
   ...
   allocate( arid(ide,jde),stat=astat ) 
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate arid; error = ',astat
...
!---------------------------------------------------------------------
      ...
      if( varname(:) == 'arid' ) then
        arid(:,:) = scale_factor * wrk_data(:,:)
      endif
...
!---------------------------------------------------------------------
      ...
      outpname = trim(out_dir)//'/'//'grid_arid.csv' !shc added out_dir
      open(unit=99,file=outpname,status='unknown')
      write(99,'(a)')"CID, ICELL, JCELL,  ARID"
