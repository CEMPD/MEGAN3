...
!--------------------------------------------------------------------
   ...
   integer :: domain
   integer :: mnth, mnth_s, mnth_e, imonth
...
!-----read landfrac----------------------------------------------
      scale_factor  = 1.
    cmonth(:) = (/ '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'/)
    do imonth  = 1, 12  !there are 12 months 
         write(*,*)'month=',imonth
         inpname       = 'soil_nitrogen_mon' // cmonth(imonth) // '.nc'
         varname       = 'nitrogen'
         missing_value = -999
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_nitrogen'
!write to NITROGEN.csv file---------------------
      CALL write_nitrogen
...
!---------------------------------------------------------------------
   allocate( mnitrogen(ide,jde,12),stat=astat ) 
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate mnitrogen; error = ',astat
...
!---------------------------------------------------------------------
      ...
      if( varname(:8) == 'nitrogen' ) then
      write(*,*)'imonth=',imonth
!Jiang
        mnitrogen(:,:,imonth) = scale_factor * wrk_data(:,:)
      endif
...
!---------------------------------------------------------------------
       ...
       if( ierr /= 0 ) then
         write(message,*) 'Failed to allocate tmp3 for nitrogen; error = ',ierr
         stop 'bio_emiss abort'
       endif
       ...
       do mnth = mnth_s,mnth_e
         do j = jds,jde
           do i = ids,ide
             ...
             if( varname(:8) == 'nitrogen' ) then
!Jiang
        mnitrogen(i,j,imonth) = scale_factor * wrk_sum
             endif
...
!---------------------------------------------------------------------
   ...
   if( ret /= nf_noerr ) then
      ...
      subroutine write_nitrogen
!---------------------------------------------------------------------
!       ... write the nitrogen  file
!---------------------------------------------------------------------
!       ... local variables
!---------------------------------------------------------------------
      ...
      integer :: dsrad,dtemp
      real :: xnitrogen(ide,jde,12)

      outpname = trim(out_dir)//'/'//'grid_NITROGEN.csv' !shc added out_dir
...
! convert from kg/m2/s to ng/m2/s
     xnitrogen(:,:,:)=mnitrogen(:,:,:)*1E+12

!Jiang      do ilat=1,jde-jds+1
!Jiang        do ilon=1,ide-ids+1
!Jiang        do im=1,12 
!Jiang      if(xnitrogen(ilon,ilat,im).gt.24.or.xnitrogen(ilon,ilat,im).lt.0.0)xnitrogen(ilon,ilat,im)=0.0
!Jiang        end do
!Jiang      end do
!Jiang      end do
!-----write new xnitrogen to output
