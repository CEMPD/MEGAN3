...
! Serena H. Chung
! 2014-05-02
! Updated for W126 Ling Huang 04/05/17
!
!--------------------------------------------------------------------
...
!----------------------------------------------------
!----------------------------------------------------
   real, allocatable :: o3(:,:)                         ! W126 daily index value
...
!-----read W126 daily index----------------------------------------------
      scale_factor  = 1.
      inpname       = 'W126_RPO36_0.3degree.nc'
      varname       = 'O3'
      missing_value = -999
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_w126'
!write to w126.csv file---------------------
      CALL write_w126
...
!---------------------------
   ...
   allocate( o3(ide,jde),stat=astat ) 
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate o3; error = ',astat
...
!---------------------------------------------------------------------
      ...
      outpname = trim(out_dir)//'/'//'grid_W126.csv' !shc added out_dir
      open(unit=99,file=outpname,status='unknown')
      write(99,'(a)')"CID, ICELL, JCELL,  W126(ppm-hours)"
