!--------------------------------------------------------------------
! This program that converts the following emissions, LAI, and PFT data into 
!    into csv files that can be used in MEGAN2.1. The potential fields are:
!       1) biogenic emissions data
!       2) LAIv
!       3) PFTs
! The program was modified from MEGAN2WRF tool developed by ACD NCAR 
! This code only works for EFglobal, LAIna2008*, and PFTna
! If you have different datasets, you will need change a few places in the code
! to read newfiles and variable names.
!--------------------------------------------------------------------
...
!----include more for offline MEGANv2.10
...
!----the following factors are assigned "1"
...
!-----Methylbutenol (ug/m2/hr)
     scale_factor = 1.0
      ...
      varname       = 'mbo22'
      CALL  megan2_bioemiss
!shc Based on email exchange with Alex Guenther on 2014-05-12, 
!shc don't use meohall200021_30sec.nc for MeOH; the PFT approacn
!shc will give the same results; see later setting in the code
!shc where msebio_meoh(:,:) is set to 1.0
!shc!-----Methanol (ug/m2/hr)
!shc!-----current no 2008 data for methanol---
!shc!-----use 2000 data instead
!shc      missing_value = -32768.
!shc      scale_factor = 1.0
!shc      inpname       = 'meohall200021_30sec.nc'
!shc      varname       = 'methanol'
!shc      CALL  megan2_bioemiss
!-----NO (ug/m2/hr)
      missing_value = 0.
      scale_factor = 1.0
      inpname       = 'nox22.nc'
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_ef210'
!write to EF210.csv file--------
      CALL write_ef210
...
!JXY ---include more emissions factors
             elseif( trim(varname) == 'myrc22' ) then
...
!---------------------------------------------------------------------
      ...
      outpname = trim(out_dir)//'/'//'EF210.csv' !shc added out_dir
...
!shc      if(MSEBIO_ISOP(ilon,ilat).gt.1000000.or.MSEBIO_ISOP(ilon,ilat).lt.0.000001)then
!shc      endif
!shc      write (99, '(3(I5,","), 2(F10.4,","),2(I,","),19(F10.4,","),1(F10.4))')cell_id,ilon-1,ilat-1,XLAT(ilon,ilat),XLONG(ilon,ilat), &
!shc                                        dsrad,dtemp,&
!shc                                        MSEBIO_ISOP(ilon,ilat),MSEBIO_MYRC(ilon,ilat),MSEBIO_SABI(ilon,ilat), &
!shc                                        MSEBIO_LIMO(ilon,ilat),MSEBIO_A_3CAR(ilon,ilat),MSEBIO_OCIM(ilon,ilat), &
!shc                                        MSEBIO_BPIN(ilon,ilat),MSEBIO_APIN(ilon,ilat),MSEBIO_OMTP(ilon,ilat), &
!shc                                        MSEBIO_FARN(ilon,ilat),MSEBIO_BCAR(ilon,ilat),MSEBIO_OSQT(ilon,ilat), &
!shc                                        MSEBIO_MBO(ilon,ilat),MSEBIO_MEOH(ilon,ilat),MSEBIO_ACTO(ilon,ilat), &
!shc                                        MSEBIO_CO(ilon,ilat),MSEBIO_NO(ilon,ilat),MSEBIO_BIDER(ilon,ilat), &
!shc                                        MSEBIO_STRESS(ilon,ilat),MSEBIO_OTHER(ilon,ilat)                            
!shc       end do
!shc      end do
