!--------------------------------------------------------------------
! Based of bio_emiss.f90_newef to process only soil arid and nonarid 
! 
! Xiaoyan Jiang
! 2019-07-12 
!
...
!----------------------------------------------------
!----------------------------------------------------
   real, allocatable :: non_arid(:,:)                         ! non_arid daily index value
...
!-----read non_arid daily index----------------------------------------------
      scale_factor  = 1.
      inpname       = 'soil_climate_non_arid.nc'
      varname       = 'non_arid'
      missing_value = -999
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_non_arid'
!write to non_arid.csv file---------------------
      CALL write_non_arid
...
!---------------------------
   ...
   allocate( non_arid(ide,jde),stat=astat ) 
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate non_arid; error = ',astat
...
!---------------------------------------------------------------------
   ...
   use area_mapper, only : area_interp2
...
!---------------------------------------------------------------------
      ...
      if( varname(:) == 'non_arid' ) then
        non_arid(:,:) = scale_factor * wrk_data(:,:)
      endif
       ...
       do mnth = mnth_s,mnth_e
         do j = jds,jde
           do i = ids,ide
             ...
             if( varname(:) == 'non_arid' ) then
               non_arid(i,j) = scale_factor * wrk_sum
             endif
           end do
         end do
       end do mnth_loop
...
!---------------------------------------------------------------------
      ...
      outpname = trim(out_dir)//'/'//'grid_non_arid.csv' !shc added out_dir
      open(unit=99,file=outpname,status='unknown')
      write(99,'(a)')"CID, ICELL, JCELL,  NON_ARID"
     ...
     do ilat=2,jde-jds
        do ilon=2,ide-ids
           if ( (ilon.gt.x0) .and. (ilon.le.(ncolsin+x0)) .and.   &
              ...
              if(non_arid(ilon,ilat)<=0.5)then
               non_arid(ilon,ilat)=0
              else
